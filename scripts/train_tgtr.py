...
def main() -> int:
    ...
    if args.resume:
        ...
        if state.get("config") != cfg or state.get("vocab") != meta["vocab"] or state.get("label_maps") != meta["label_maps"]:
            raise ValueError("TGTR resume requires the same configuration, vocabulary and label maps")
    ...
    checkpoint_payload = (
        {
            ...
            "config": cfg,
            "architecture": cfg.get("architecture", "TennisVAR-TGTR"),
            "graph_source": cfg.get("data", {}).get("graph_source", "region_fusion"),
            "event_backend": event_backend,
            "feature_root": str(feature_root),
            "seed": seed,
