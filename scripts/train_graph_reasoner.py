...
from rallymotionreasoner.config import DEFAULT_CONFIG
from rallymotionreasoner.configs import (
    dump_simple_yaml,
    graph_reasoner_feature_root,
...
from rallymotionreasoner.training.eval import evaluate_internal
from rallymotionreasoner.training.losses import compute_loss


def build_model(cfg: dict[str, Any], vocab_size: int, label_maps: dict[str, dict[str, int]]) -> RallyGraphReasoner:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train dataset-neutral RallyMotionReasoner-RGR on predicted event graphs.")
    parser.add_argument("--config", type=Path, default=DEFAULT_CONFIG)
    parser.add_argument("--experiment-config", type=Path, default=Path("configs/rallymotionreasoner.yaml"))
    ...
    cfg = load_graph_reasoner_config(args.experiment_config, overrides)
    ...
    default_run = f"{cfg.get('experiment_name', 'rgr_vl')}_{time.strftime('%Y%m%d_%H%M%S')}_seed{seed}"
    ...
    if output_dir.exists():
        if not args.resume:
            raise FileExistsError(f"refusing to overwrite RGR run: {output_dir}")
        if final_checkpoint_path.exists():
            raise FileExistsError(f"RGR run is already complete: {final_checkpoint_path}")
        if not trainer_state_path.is_file():
            raise FileNotFoundError(f"RGR run has no resumable trainer state: {trainer_state_path}")
    else:
        if args.resume:
            raise FileNotFoundError(f"RGR run does not exist for --resume: {output_dir}")
        ...
        write_json(
            output_dir / "model_card.json",
            {
                ...
                "visual_backbone": cfg["feature_extraction"]["feature_set"],
                "claim_boundary": "RGR is an evidence selector; the structured-prompt baseline is not claimed as joint end-to-end fusion.",
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        trainer_state = {
            "schema": "rallymotionreasoner.rgr_trainer.v1",
    ...
    if best_state is None:
        raise RuntimeError("RGR training completed without a valid best checkpoint")
    ...
    checkpoint_payload = (
        {
            ...
            "config": cfg,
            "architecture": cfg.get("architecture", "RallyMotionReasoner-RGR"),
            "graph_source": cfg.get("data", {}).get("graph_source", "motion_region"),
            "event_backend": event_backend,
            "feature_root": str(feature_root),
            "seed": seed,
