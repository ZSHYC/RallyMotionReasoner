...
def validate_paper_config(config: dict[str, Any]) -> None:
    ...
    errors = []
    if str(data.get("graph_source")) != "motion_region":
        errors.append("data.graph_source must be motion_region (predicted events)")
    ...
    if errors:
        raise ValueError("invalid RallyMotionReasoner paper configuration: " + "; ".join(errors))


def load_graph_reasoner_config(experiment_config: Path | None = None, overrides: list[str] | None = None) -> dict[str, Any]:
    path = Path(experiment_config or "configs/rallymotionreasoner.yaml")
...
def graph_reasoner_feature_root(paths: dict[str, Path], config: dict[str, Any]) -> Path:
