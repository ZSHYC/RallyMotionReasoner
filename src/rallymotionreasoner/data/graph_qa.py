...
from rallymotionreasoner.evaluation.matching import optimal_temporal_matching
from rallymotionreasoner.graph_reasoning.graph_transformer import EDGE_TYPE_TO_ID
from rallymotionreasoner.io import read_jsonl
from rallymotionreasoner.schema import ANSWER_TYPES, ANSWERABILITY, CAUSAL_STRENGTHS, OBSERVED_EFFECTS, answer_payload
...
def graph_file(paths: dict[str, Path], split: str, source: str, cfg: dict[str, Any] | None = None) -> Path:
    ...
    if source == "motion_region":
        root = dataset_relative_root(paths, cfg, "graph_root_name", "graphs_motion_region")
        return root / f"sgtr_graph_motion_region_{name}_{split}.jsonl"
...
def load_visual_features(
    ...
    *,
    ...
    if strict:
        if (
            data.get("schema") != "rallymotionreasoner.rgr_event_features"
            or data.get("source") != "motion_region_predicted"
...
class GraphQADataset(Dataset[EncodedItem]):
    def __init__(
        ...
        *,
        ...
        graph_source: str = "motion_region",
...
def make_graph_datasets(
    ...
    graph_source = str(data_cfg.get("graph_source", "motion_region"))
    ...
    meta = {
        ...
        "evidence_alignment": "one_to_one_temporal" if graph_source == "motion_region" else "gold_shot_id",
    }
