...
from .region_features import RegionFeatureExtractor, normalize_trajectory
from .region_model import TrajectoryExpert, VisualExpert
from .runtime import EventRuntimeOutput
...
def _decode(scores: Any, frames: list[int], fps: float, threshold: float, radius: int) -> list[DecodedEvent]:
    ...
    for event_type, column in zip(EVENT_TYPES, range(len(EVENT_TYPES)), strict=True):
        kept: list[int] = []
        candidates = sorted(
...
class RegionEventPredictor:
    """Direct runtime for the published trajectory/visual expert directory."""

    backend = "region_fusion"
    ...
    def __init__(
        ...
        *,
        ...
        self.checkpoint = {
            "schema": "tennisvar.region_fusion.v1",
    ...
    def predict(
        ...
        *,
        ...
        frames = [int(value) for value in (frame_indices or range(len(paths)))]
