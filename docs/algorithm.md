...
`src/rallymotionreasoner/features/ball_trajectory.py` accepts TrackNet JSON or CSV with finite, ordered points inside their declared source dimensions, then normalizes them into fixed temporal rows. Raw-video inputs also require track dimensions to match the decoded video; preprocessed frame directories can retain the original track dimensions. Each row contains ball position, motion derivatives, visibility, and a validity indicator. The trajectory encoder models short-term movement and preserves the distinction between a valid temporal slot and a visible ball observation. Its released expert weights retain a 779-column projection, with the first 11 columns used for the 11 trajectory inputs.
...
The runtime in `src/rallymotionreasoner/event_detection/runtime.py` loads `trajectory_expert.pt` and `visual_expert.pt`, evaluates their fixed-length, zero-padded windows, fuses the expert scores, and decodes hit and bounce events with separate five-frame NMS for each type. Non-finite scores are rejected. Bounce is a graph cue, not a player stroke. Raw videos use constant frame rate and lossless temporary PNG frames; frame directories need an explicit FPS.
...
Ball-position, ball-visibility, and contact-time heads are optional auxiliary targets. They receive supervision only when `feature_extraction.visual_supervision_path` is configured; the default configuration does not provide that file. Visibility has its own annotation mask, so an explicitly invisible ball is a supervised negative sample even without position coordinates. These target values are not fed into the RGR encoder.

RGR uses the checkpoint's `max_strokes` and `top_k` limits in validation and inference. `top_k` is between 1 and the 32-frame Qwen video budget. Evidence is ranked, capped at `top_k`, then filtered by the calibrated threshold; key-action evaluation permits an empty prediction and counts only evidence above that threshold. Evidence-frame tolerance is calibrated at 25 FPS and scaled to each graph's FPS; key-action targets are restricted to evidence targets. Strict feature loading matches each shot ID and frame against the event graph. The default `time_bias_version: 2` gives zero temporal distance its own attention bucket. Checkpoints without that field use version 1 timing; checkpoints without `top_k` use 8.

## 4. Evidence routing and generation

The evidence router ranks candidate strokes and key actions using the question representation and RGR state. Key-action scores combine evidence and conditional key probabilities. Candidates below the RGR evidence threshold are withheld from generation; when none qualify, the pipeline returns an unanswerable result. Otherwise, the generator receives validated candidate IDs, selected evidence frames, global context, and predicted event information. Frame selection keeps evidence centers, samples up to 16 global frames and four frames within eight source frames of each candidate, and caps the total at 32. Local allocation is shared across candidates when the budget fills. Qwen receives the selected frames with their original frame indices and source FPS for timestamps. Training rows require matching `e2e_metadata.video_frame_indices` and `video_fps`, candidate frames present in the video frame list, the canonical predicted-candidate prompt without a system message, and a data report covering both splits. Assistant targets cite only prompt candidate IDs, use their corresponding frames, and cite evidence for answerable outputs. Targets without evidence use the online abstention fields: empty answer, null levels, unknown effect, and insufficient causal strength. An adapter manifest declares the prompt, sampling, and source-timing contract; an adapter without a compatible contract cannot be loaded. Resume training also requires the same data paths, report path, row counts, and truncation limits. Generated answers without valid candidate evidence are unanswerable with empty answer text. Output validation is defined in `src/rallymotionreasoner/schema.py`.
